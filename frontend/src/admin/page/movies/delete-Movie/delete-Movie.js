...
const Delete_Movie = ({ movie, onDelete }) => {
    ...
    const handleConfirm = () => {
        if (inputValue.trim() === movie.MovieNameVietnamese) {
            console.log('Đã xóa phim:', movie.MovieNameVietnamese);
            onDelete?.(movie);
    ...
    return (
        <div className="flex gap-6 p-6 max-w-4xl mx-auto bg-white rounded-2xl shadow-lg mt-10 relative">
            ...
            <div className="flex-1 flex flex-col gap-3 items-start">
                <img src={movie.file} alt={movie.MovieNameVietnamese} className="w-full h-60 object-cover rounded-xl" />
                <h2 className="text-xl font-bold">{movie.MovieNameVietnamese}</h2>
                <p><strong>Số tập:</strong> {movie.NumberOfEpisodes}</p>
                ...
                <AnimatePresence>
                    {showConfirm && (
                        ...
                        >
                            ...
                            <p className="mb-2">Bạn có chắc muốn xóa phim <strong>{movie.MovieNameVietnamese}</strong>?</p>
                            ...
                            <input
                                ...
                                placeholder={`Nhập "${movie.MovieNameVietnamese}"`}
